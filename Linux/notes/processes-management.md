...
1: ps aux = Shows all running processes with details like CPU and memory usage.



Example Output:

$ ps aux root 1 0.0 0.1 169260 1100 ? Ss 10:00 0:02 /sbin/init user 1324 1.2 3.5 812344 28900 ? Sl 10:05 0:10 python3 app.py 
...
2: top = Displays real‑time system activity, including active processes and resource usage.



Example Output:

$ top PID USER PR NI VIRT RES SHR S %CPU %MEM TIME+ COMMAND 1324 user 20 0 812344 28900 1200 S 12.3 3.5 0:10.23 python3 
...
3: htop = Interactive version of top with easier navigation and colour‑coded output (if installed).



Example Output:

$ htop (Interactive colourful dashboard showing CPU, memory, processes) 
...
4: kill <PID> = Sends a signal to stop a process using its process ID.
...
5: kill -9 <PID> = Force‑kills a process immediately using the SIGKILL signal.
...
6: pkill <name> = Stops processes by matching their name instead of using a PID.
...
7: systemctl status <service> = Shows the current status of a system service.
...
8: systemctl restart <service> = Restarts a system service to apply changes or recover from issues.
