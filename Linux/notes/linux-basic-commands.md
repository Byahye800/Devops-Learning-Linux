...
1: pwd = Shows the full path of the directory you are currently in.

Example Output: $ pwd /home/ubunto/projects 
...
2: ls = Lists files and directories in the current location.

Example Output: $ ls app.py logs config.yaml scripts 
...
3: ls -lah = Lists files with details, including hidden files, in human‑readable format.

Example Output: $ ls -lah total 20K drwxr-xr-x 3 user user 4.0K Feb 4 10:21 . drwxr-xr-x 5 user user 4.0K Feb 4 10:20 .. -rw-r--r-- 1 user user 1.2K Feb 4 10:21 config.yaml 
...
4: cd <directory> = Changes the current directory.

Example Output: $ cd /var/log 
...
6: cd ~ = Returns to your home directory.

Example Output: $ cd ~ $ pwd /home/ubunto 
...
Example Output: $ touch notes.txt $ ls notes.txt 
...
2: mkdir <folder> = Creates a new directory.

Example Output: $ mkdir\_backups 
...
3: mkdir -p <path> = Creates nested directories without errors if they already exist.

Example Output: $ mkdir -p /opt/app/releases/v1 
...
4: cp <source> <destination> = Copies a file.

Example Output: $ cp config.yaml config.yaml.bak 
...
5: mv <source> <destination> = Moves or renames a file.

Example Output: $ mv app.log old-app.log 
...
6: rm <file> = Deletes a file permanently.

Example Output: $ rm old-app.log 
...
1: cat <file> = Displays the entire contents of a file.

Example Output: $ cat config.yaml port: 8080 environment: production 
...
2: less <file> = Opens a scrollable viewer for large files.

Example Output: $ less /var/log/syslog 
...
3: head -n 20 <file> = Shows the first 20 lines of a file.

Example Output: $ head -n 20 app.log \[INFO] Starting service... \[INFO] Loading config... 
...
4: tail -n 20 <file> = Shows the last 20 lines of a file.

Example Output: $ tail -n 20 app.log \[INFO] Service started successfully 
...
5: nl  <file> = Displays a file with line numbers added automatically.

Useful when referencing specific lines.

Example Output: $ nl script.sh 1 #!/bin/bash 2 echo "Deploying..." 
...
6: more <file> = Shows a file one page at a time, similar to less but more basic.

Example Output: $ more README.md 

DevOps Usage:

Used to view long files on systems.
